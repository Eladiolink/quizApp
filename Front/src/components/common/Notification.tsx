...
export function NotificationClient({ message, type = "success", onClose }: NotificationProps) {
  const [isVisible, setIsVisible] = useState(true);
  ...
  return (
    <div
      className={`
        fixed top-[80px] right-4 px-4 py-2 rounded shadow text-black z-50 transition-all duration-500 ease-in-out
        ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full"}
        slide-in-start
        bg-white
        border-b-4
